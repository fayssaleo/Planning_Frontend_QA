<template>
  ...
  <v-row style="width: 100%;     height: 79vh;background-color: white !important" class="mt-0">
    <v-data-table :headers="headers" :items="thisPlanningBoxes" :custom-sort="customSort"
      ...
      :items-per-page="thisPlanningBoxesFiltered.length" style="width: 99%; margin: 0 auto"
      :class="[
        ...
        <tr class="rowToEdit" :class="editMode ? 'clickableCase' : ''">
          <td width="15%" @click="editMode ? item[0] = clickOnThePlanningWorkDriver(item[0], rowIndex, false) : ''"
            ...
            <span :style="{ 'background-color': item[0].shift_id != actualShift.id ? '#152855ba !important' : '' }" class="fullname" :class="item[0].shift_id != actualShift.id ? 'otherShiftBox' : ''"
              ...
              {{ item[0].lastname.toUpperCase() }}<span style="color:#80c7ff" v-if="item[0].shift_id != actualShift.id"> ({{ getShiftByIndex(item[0].shift_id)}})</span>


              <span class="otherShift_wh" >{{Math.trunc(item[0].workingHours) }}<span> WH</span></span>
...
">
          <v-row class="confirmDialogInterfaceHeader" style="height: 0px;">
            <v-col v-if="selectedChangeEDSow == 0 ? 'selectedChangeEDSow' : ''" cols="12" class="children" :class="selectedChangeEDSow == 0 ? 'selectedChangeEDSow' : ''"
...
export default {
  ...
  methods: {
    ...
    calculateThisDriverHours(driverIndex){
      ...
      for (let index = 0; index < tempHeaders.length; index++) {
        console.log("driverIn"+index+"dex[index+1]! ",driverIndex[index+1])
        if(driverIndex[index+1]!="P" && driverIndex[index+1]!="DP"){
          periodes.push(tempHeaders[index])
        }
      }
      console.log("periodes : ",periodes);
      console.log("calculateTotalHours2 : ",this.calculateTotalHours2(periodes));
    ...
    switchEquipmentFromRealPlanning_H(tempPlanning, equipmentOld, equipmentNew) {
      let tempPlanning2 = [...JSON.parse(JSON.stringify(tempPlanning))];
      console.log("yardddd this.equipmentsNotInPlanningWithCheck  jdida:", JSON.parse(JSON.stringify(this.equipmentsNotInPlanningWithCheck)));
  ...
  function forceInsertion2(thePlanning, stsListtoBegin) {
    ...
    console.log("wslna", thePlanningToReduce);
    for (let STSIndex = 0; STSIndex < stsListtoReduce.length; STSIndex++) {
      for (
        let intervalndex = 0;
        ...
        if (columnIndex != -1) {
          if (STSIndex == 0 && intervalndex == 1) {
            console.log("wslna", columnIndex);
            console.log("wslna sts ", stsListtoReduce[STSIndex].matricule);
            console.log(
              "wslna time ",
              stsListtoReduce[STSIndex].intervals[intervalndex]
            );
          }
        ...
        for (let column_Indexor = 0; column_Indexor < 8; column_Indexor++) {
          ...
          if (copyPlanningToRe != null) {
            if (
              ...
              console.table(copyPlanningToRe);
              ...
              if (rst) {
                console.log("else of canBe :", copyPlanningToRe_2);
                console.table(copyPlanningToRe_2);
      ...
      for (let index = 1; index < thePlanningToReduce_2.length; index++) {
        if (index_ != index) {
          ...
          if (tryToBelowMany_row_rst) {
            console.log(
              "yesss tryToBelowMany_row_rst : oldIndex : " +
                index_ +
                "  newIndex " +
                index
            );
            console.table(tryToBelowMany_row_rst);
                ...
                if (tryToBelowMany_row_rst_2) {
                  for (
                    ...
                    if (
                      ...
                      console.log(
                        "FINAlY ONE tryToBelowMany_row_rst : oldIndex : " +
                          index_ +
                          "  newIndex " +
                          index_2
                      );
          ...
          if (tryToBelowMany_row_rst_2) {
            for (
              ...
              if (
                ...
                console.log(
                  "FINAlY ONE thePlanningToReduce_2 : oldIndex : " +
                    index_ +
                    "  newIndex " +
                    index_2
                );
  ...
  function tryToBelowMany_row(
    ...
    if (column == 0) {
      ...
      if (newIndex == 2) {
        console.log("thePlanningToReduce_2 after erase", thePlanningToReduce_2);
      }
      for (
        let addIndex = column;
  ...
  function balanceTheWH_FEQUP(thePlanning) {
    for (let index = 1; index < thePlanning.length; index++) {
      ...
      console.log(getDriverinalWOrkingHours(index));
      if (countWH(thePlanning[index]) < mustWork) {
        console.log(thePlanning[index]);
          ...
          if (thePlanning[index][columnIndex] == "-") {
            for (
              ...
              if (
                ...
                if (
                  columnIndex == 1 &&
                  thePlanning[last_index][columnIndex] == "STS02"
                ) {
                  console.log(
                    "thePlanning[last_index][columnIndex]",
                    thePlanning[last_index][columnIndex]
  ...
  function insertWithPillars_fusion(thePlanningToReduce,index,index_origin, columnIndex, missinOne) {
        ...
        if(canBeInserted2(thePlanningToReduce_2[index], missinOne, columnIndex)){
          console.log("from first shoot :")
          thePlanningToReduce_2[index][columnIndex]=missinOne;
          console.table(thePlanningToReduce_2)
          return thePlanningToReduce_2;
        }
        for (let column_Indexor = 0; column_Indexor < 8; column_Indexor++) {
          ...
          if(copyPlanningToRe && (firstElementElement || lastElement)){
            ...
            if(canBeInserted2(copyPlanningToRe[index], missinOne, columnIndex)){
              console.log("from copyPlanningToRe  :")
              copyPlanningToRe[index][columnIndex]=missinOne;
              console.table(copyPlanningToRe)
  ...
  function findCouple(thePlanning, stsToCouple,index_,columnIndex){
    ...
    for (let index = 1; index < thePlanning.length; index++) {
          ...
          if (columnIndex == 0 && thePlanning[index][columnIndex + 1] == stsToCouple) {
            console.log("---------------- FOUND : "+stsToCouple+" AT "+shift[columnIndex]+" IN : -----------------");
  ...
  function setAllDriversPlanning() {
    ...
    console.log(
      "stsListtoInsert :",
      JSON.parse(JSON.stringify(stsListtoBegin))
    );
    ...
    console.log("after - bestenhancment");
    stsListtoBegin = findMissingHours3(stsListtoBegin, thePlanning);
    console.log(
      "stsListtoInsert :",
      JSON.parse(JSON.stringify(stsListtoBegin))
    ...
    console.log("after - simpleInsertion");
    stsListtoBegin = findMissingHours3(stsListtoBegin, thePlanning);
    console.log(
      "stsListtoInsert :",
      JSON.parse(JSON.stringify(stsListtoBegin))
    ...
    console.log("after - multipleInsertion");
    stsListtoBegin = findMissingHours3(stsListtoBegin, thePlanning);
    console.log(
      "stsListtoInsert :",
      JSON.parse(JSON.stringify(stsListtoBegin))
    ...
    stsListtoBegin = findMissingHours3(stsListtoBegin, thePlanning);
    console.log("matrix");
    //console.table(smartInsert([...JSON.parse(JSON.stringify(thePlanning))], [...stsListtoBegin]))
    //stsListtoBegin=[...stsList];
    //console.log(
    //  "stsListeeeetoInsert :",
    //  JSON.parse(JSON.stringify(stsListtoBegin))
    //);
    ...
    thePlanning.unshift(shift);
    console.log("after - balanceTheWH");
    stsListtoBegin = findMissingHours3(stsListtoBegin, thePlanning);
    console.log(
      "stsListtoInsert :",
      JSON.parse(JSON.stringify(stsListtoBegin))
  ...
  return output;
    ...
    async clickFinish() {
      ...
      if (this.reloadPlanning) {
        ...
        let equipmentInPlanning_now = this.equipmentsInThePlanning2();
        console.log("hna hna ",equipmentInPlanning_now)
        ...
        console.log("tempPlanning :", tempPlanning_2);
        tempPlanning_2 = this.switchAndDeleteFromHistory(tempPlanning);


        tempPlanning = tempPlanning_2;

        console.log("tempPlanning after delete :", JSON.parse(JSON.stringify(tempPlanning)))
        console.log("equipmentInPlanning_now :", JSON.parse(JSON.stringify(equipmentInPlanning_now)))
        console.log("equipmentInPlanning_now2 :", JSON.parse(JSON.stringify(equipmentInPlanning_now2)))
        console.log("tepEquipmentInThePlanning :", JSON.parse(JSON.stringify(tepEquipmentInThePlanning)))
    ...
    fixItForSTSLogic1(equipmentInPlanning_now) {
      ...
      console.log("yarbi salam this.tempPlanning_forShow  jdida:", JSON.parse(JSON.stringify(this.tempPlanning_forShow)));
    ...
    deleteEquipmentNotSelectedInPlanning(tempPlanning, equipmentsInPlanningWithCheck) {
      console.log("tempPlanning :", JSON.parse(JSON.stringify(tempPlanning)))
      console.log("equipmentsInPlanningWithCheck :", JSON.parse(JSON.stringify(equipmentsInPlanningWithCheck)))
